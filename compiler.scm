...
(define (compile-map exp target linkage)
  (let ((map-cond   (make-label 'map-cond  ))
        (map-return (make-label 'map-return))
        (map-push   (make-label 'map-push  ))
        (proc-code (compile (map-proc exp) 'proc 'next))
        (list-code (compile (map-list exp) 'unev 'next)) )
    (end-with-linkage linkage
      (preserving '(env continue)
        (append-instruction-sequences
          list-code proc-code)
        (preserving '(continue)
          (append-instruction-sequences
            (make-instruction-sequence
              '(temp1 unev) '(temp1 unev) '(temp1 unev)
              `((assign temp2 (reg proc))
                (assign temp1 (const ()))
                (assign unev (op reverse) (reg unev)) ))
            map-cond
            (make-instruction-sequence
              '(argl unev) '(argl unev) '(argl unev)
            ...
            (compile-procedure-call 'val map-push)
            map-push
            (make-instruction-sequence
              '(temp1 val) '(temp1) '(temp1)
...
(define (compile-apply exp target linkage)
  (let ((proc-code     (compile (apply-proc exp) 'proc 'next))
        (operand-codes (compile (apply-args exp) 'argl 'next)) )
    (preserving '(env continue)
                proc-code
