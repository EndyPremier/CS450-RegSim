...
(define (make-new-machine)
    (let ((pc (make-register))
      ...
      (let ((the-ops
        ...
        (define (allocate-label label pos)
          (if (assoc label label-table)
        ...
        (define (reference label)
          (let ((label-tag (assoc label label-table)))
            (if label-tag
                (let ((pos (assoc (cdr label-tag) the-instruction-sequence)))
                  (member pos the-instruction-sequence) )
                (error "Unknown label: " label) )))
        ;; EXECUTION
        (define (execute)
          (define (at-break? line-num)
            (assoc line-num breakpoint-table))
          (let ((insts (get-contents pc)))
            (cond ((null? insts) 'done)
                  ((at-break? (caar insts))
                    (let ((bp (assoc (caar insts) breakpoint-table)))
                      (display "Breaking at ")
                      (display (cadr bp)) (display ", ")
                      (display (cddr bp)) (newline) ))
                  (else ((instruction-execution-proc (car insts)))
                        (execute) ))))
        (define (proceed)
          (let ((insts (get-contents pc)))
            (if (null? insts)
                (begin (display "Please restart with start.") (newline))
                (begin ((instruction-execution-proc (car insts)))
                       (execute) ))))
        ;; BREAKPOINT
        (define (breakpoint message)
          (define (line-num-from-label label n)
            (let ((line-at (assoc label label-table)))
              (cond ((not line-at) (error "Unknown label: " label))
                    ((<= n 0) (error "n must be greater than 0."))
                    ((> (+ (cdr line-at) n -1)
                        (length the-instruction-sequence) )
                      (error "Invalid breakpoint position") )
                    (else (+ (cdr line-at) n -1)) )))
          (define (set-breakpoint label n)
            (let ((pos (line-num-from-label label n)))
              (if (member pos breakpoint-table)
                  (error "Multiply defined breakpoint: " (list label n))
                  (set! breakpoint-table
                        (cons (cons pos (cons label n))
                              breakpoint-table) ))))
          (define (cancel-breakpoint label n)
            (let ((pos (line-num-from-label label n)))
              (define (iter alist)
                (cond ((null? alist) '())
                      ((equal? (caar alist) pos) (cdr alist))
                      (else (cons (car alist) (iter (cdr alist)))) ))
              (set! breakpoint-table (iter breakpoint-table)) ))
          (cond ((eq? message 'set)    set-breakpoint)
                ((eq? message 'cancel) cancel-breakpoint)
                ((eq? message 'list)   breakpoint-table)
                ((eq? message 'clear)  (set! breakpoint-table '())) ))
        ;; DISPATCH
        (define (dispatch message)
          (cond ((eq? message 'start)
                ...
                ;; DEBUG
                ((eq? message 'instruction) the-instruction-sequence)
                ((eq? message 'label-pos) label-table)
                ((eq? message 'reference) reference)
                ;; REQUIRED DEBUG
                ((eq? message 'allocate-label) allocate-label)
...
(define (extract-labels text receive num machine)
  (if (null? text)
...
(define (make-instruction text num)
  (cons num (cons text '())) )
...
(define (make-label-entry label-name insts machine num)
  ;;; label position for breakpoints
  ((machine 'allocate-label) label-name num)
  ;;; main output
...
(define (label-reference machine label)
  ((machine 'reference) label))
...
(define (get-stack machine)
  ((machine 'stack) 'curr-stack) )

(define (get-breakpoints machine)
  ((machine 'breakpoint) 'list) )
