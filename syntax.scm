...
(define (cond-empty? clauses) (null? clauses))
(define (cond-curr-clause clauses) (car clauses))
(define (cond-next-clauses clauses) (cdr clauses))
...
;;; let special form -> lambda equivalent
...
(define (let-bindings-vars  exp) (map car  (let-bindings exp)))
(define (let-bindings-inits exp) (map cadr (let-bindings exp)))

(define (let->appl exp)
  (cons (cons 'lambda (cons (let-bindings-vars exp) (let-body exp)))
        (let-bindings-inits exp) ))


;;; or special form -> if equivalent

(define (or? exp) (tagged-list? exp 'or))

(define (or-tests exp) (cdr  exp))
(define (or-curr  exp) (cadr exp))
(define (or-next  exp) (cddr exp))

(define (or->if exp)
  (if (null? (or-tests exp))
      #f
      (list 'if (or-curr exp)
                (or-curr exp)
                (cons 'or (or-next exp)) )))
...
(define (apply-args exp) (caddr exp))
